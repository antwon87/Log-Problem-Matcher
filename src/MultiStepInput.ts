...
export class MultiStepInput {
   ...
   async showQuickPick<T extends vscode.QuickPickItem, P extends QuickPickParameters<T>>({ title, step, totalSteps, items, activeItems, selectedItems, ignoreFocusOut, placeholder, canSelectMany, buttons, shouldResume }: P) {
      ...
      try {
         return await new Promise<T[] | (P extends { buttons: (infer I)[] } ? I : never)>((resolve, reject) => {
            ...
            disposables.push(
               ...
               // input.onDidChangeSelection(items => input.items = items),
               input.onDidAccept(() => resolve(input.selectedItems.map(foo => foo))),
               input.onDidHide(() => {
                  (async () => {
